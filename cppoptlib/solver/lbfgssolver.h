...
class LbfgsSolver : public ISolver<ProblemType, 1> {
  public:
    ...
    void minimize(ProblemType &objFunc, TVector &x0) {
        ...
        do {
            const Scalar relativeEpsilon = static_cast<Scalar>(1e-6) * std::max<Scalar>(static_cast<Scalar>(1.0), x0.norm());
            ...
            if (descent > -1e-6 * relativeEpsilon) {
                q = -1 * grad;
                iter = 0;
                alpha_init = 1.0;
            }
            ...
            // find steplength
            Scalar rate = MoreThuente<ProblemType, 1>::linesearch(x0, -q,  objFunc, alpha_init) ;
            // update guess
//            objFunc.savePreviousStatus(x0);
            if(rate < 1e-8)
                rate = 1e-8;
            x0 = x0 - rate * q;
            grad_old = grad;
//            objFunc.update(x0);
            ...
            x_old = x0;
            std::cout << "iter: "<<globIter<< ", f = " <<  objFunc.value(x0) << ", ||g||_inf "<<grad.template lpNorm<Eigen::Infinity>()<< ", Rate: "<<rate<<std::endl;
