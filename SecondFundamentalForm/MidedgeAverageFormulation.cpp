...
static void testSecondFundamentalFormEntries(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F)
{
    ...
    {
        int face = uni(rng);
        Eigen::Matrix<double, 3, 18> deriv;
        std::vector<Eigen::Matrix<double, 18, 18> > hess;
        Eigen::Vector3d b = secondFundamentalFormEntries(mesh, V, face, &deriv, &hess);
        ...
        for(int j=0; j<3; j++)
        {
            for(int k=0; k<3; k++)
            {
                ...
                Eigen::Matrix<double, 3, 18> derivpert;
                Eigen::Vector3d bpert = secondFundamentalFormEntries(mesh, Vpert, face, &derivpert, NULL);
                ...
                Eigen::Matrix<double, 3, 18> findiffhess = (derivpert-deriv)/1e-6;
                for(int l=0; l<3; l++)
                {
